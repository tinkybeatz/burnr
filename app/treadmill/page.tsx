...
export default function Page() {
  ...
  const calculate = useCallback(() => {
    ...
    // ACSM 2021 metabolic equations (flat, grade = 0)
    const speedMmin = (speedKmh * 1000) / 60
    const vo2 =
      speedKmh < WALK_THRESHOLD_KMH
        ? 0.1 * speedMmin + 3.5   // walking
        : 0.2 * speedMmin + 3.5   // running
    const MET = Math.max(2.5, vo2 / 3.5)
    const calories = Math.round(MET * weightKg * (durationMin / 60))

    // Lindberg (2003) step length
    const heightM = heightCm / 100
    const stepLengthM =
      speedKmh < WALK_THRESHOLD_KMH
        ? 0.413 * heightM   // walking
        : 0.72 * heightM    // running
    ...
    weight, height, distance, speed, timeMins, speedTime,
